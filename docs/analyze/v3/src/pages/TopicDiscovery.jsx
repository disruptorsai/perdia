...
Return 10-15 content ideas that would resonate with GetEducated's audience.`;
  ...
  const handleGenerateArticle = (idea) => {
    navigate(createPageUrl('ArticleGenerator') + 
      `?title=${encodeURIComponent(idea.title)}&type=${idea.content_type}&keywords=${encodeURIComponent(idea.keywords?.join(', ') || '')}`
    );
  };
          ...
          <TabsContent value="approved" className="mt-6 space-y-4">
            {approvedIdeas.length === 0 ? (
              ...
              approvedIdeas.map((idea) => (
                <Card key={idea.id} className="border-none shadow-lg">
                  <CardContent className="p-6">
