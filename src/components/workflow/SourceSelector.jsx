/**
 * PERDIA EDUCATION - SOURCE SELECTOR
 * Multi-source AI-powered topic discovery with duplicate detection
 */
...
import { Article, ContentIdea, InvokeLLM } from "@/lib/perdia-sdk";
...
export default function SourceSelector({ onClose, onComplete }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const topicText = customTopic ? ` focusing on ${customTopic}` : "";
...
- Use "Latest", "Current", "This Year", or specific current year (${currentYear}) when appropriate

Sources to consider: ${sourcesText}${topicText}
${topPerformersText}
...
Return as JSON array with this structure:
{
  "ideas": [
    {
      ...
      // Filter out duplicates using semantic similarity check
      const uniqueIdeas = [];
      ...
      setGeneratedIdeas(uniqueIdeas);
      setIsGenerating(false);

      if (uniqueIdeas.length === 0) {
        toast.warning("All generated ideas were too similar to existing content. Try a different topic focus.");
      } else {
        toast.success(`Generated ${uniqueIdeas.length} unique ideas!`);
      }
  ...
  const handleSaveIdeas = async () => {
    if (selectedIdeas.length === 0) {
      toast.error("Please select at least one idea to add to the queue");
      return;
    }
    ...
    try {
      ...
      // Create all ideas
      for (const ideaData of ideasToSave) {
        await ContentIdea.create(ideaData);
      }

      queryClient.invalidateQueries({ queryKey: ['workflow-ideas'] });
      queryClient.invalidateQueries({ queryKey: ['content-ideas-wizard'] });
      toast.success(`Added ${ideasToSave.length} ideas to the queue!`);
      onComplete();
