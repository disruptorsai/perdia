...
export default function AITraining() {
  ...
  const { data: trainingData = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await TrainingData.find({}, {
...
Be specific and actionable. Focus on the most impactful improvements.`;
    ...
    onSuccess: (improvements) => {
      ...
      toast.success('AI Training Complete!', {
        description: improvements.implementation_summary
      });
    },
    onError: (error) => {
      setIsTraining(false);
  ...
  const handleApplyTraining = () => {
    ...
    if (pendingCount === 0) {
      toast.error('No training data available to apply. Complete some article revisions first.');
      return;
    }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
              ...
              >
                <div className="text-center space-y-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p>✓ Analyzing editorial feedback</p>
                    <p>✓ Identifying improvement patterns</p>
                    <p>✓ Updating system prompts</p>
                  </div>
        ...
        >
          ...
          <p className="text-gray-600 mt-1">
            Review editorial feedback and apply training to improve AI
          </p>
        </motion.div>
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <h3 className="font-bold text-lg mb-2">Ready to Apply Training</h3>
                  <p className="text-purple-100 text-sm">
                    You have {pendingData.length} training example(s) ready to improve the AI. Click to analyze feedback and update system prompts.
                  </p>
                </div>
                <Button
                  onClick={handleApplyTraining}
                  disabled={isTraining}
                  className="bg-white text-purple-700 hover:bg-purple-50 gap-2 flex-shrink-0"
                >
          ...
          <TabsContent value="pending" className="mt-6 space-y-4">
            {isLoading ? (
              ...
              <Card className="p-12 text-center border-none shadow-lg bg-white">
                ...
                <p className="text-gray-500">Training data is created when articles are revised based on editorial feedback</p>
              </Card>
            ) : (
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-lg text-gray-900 mb-2">{data.article_title}</h3>
                          <p className="text-sm text-gray-600 mb-3">{data.lesson_learned}</p>
                          <div className="flex items-center gap-3 flex-wrap">
                            <Badge variant="outline" className={`${patternColors[data.pattern_type] || patternColors.other} border`}>
                          ...
                          {data.feedback_items && data.feedback_items.length > 0 && (
                            <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                              <p className="text-xs font-semibold text-gray-700 mb-2">
                                {data.feedback_items.length} feedback item(s)
                              </p>
          ...
          <TabsContent value="applied" className="mt-6 space-y-4">
            {appliedData.length === 0 ? (
              ...
              appliedData.map((data) => (
                <Card key={data.id} className="border-none shadow-lg bg-white">
                  <CardContent className="p-6">
        ...
        <Card className="border-none shadow-lg bg-blue-50 border-blue-200">
          <CardContent className="p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-blue-900 mb-1">How AI Training Works</h4>
                <p className="text-sm text-blue-800">
                  When articles are revised based on editorial feedback, training data is automatically created.
                  Applying training analyzes all feedback patterns and updates AI prompts system-wide to prevent similar issues in future articles.
                </p>
              </div>
            </div>
          </CardContent>
