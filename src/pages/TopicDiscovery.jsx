...
import { ContentIdea, InvokeLLM } from "@/lib/perdia-sdk";
import {
  Search,
  TrendingUp,
  Sparkles,
...
export default function TopicDiscovery() {
  ...
  const { data: contentIdeas = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await ContentIdea.find({}, {
...
2. Brief description
3. Content type (ranking, career_guide, listicle, guide, faq)
4. Keywords (array)
...
Return 10-15 content ideas that would resonate with GetEducated's audience.`;
          ...
          properties: {
            ideas: {
              ...
              items: {
                ...
                required: ["title", "description", "content_type", "keywords"]
              }
            }
          },
          required: ["ideas"]
    ...
    onSuccess: async (ideas) => {
      ...
      await Promise.all(
        ideas.map(idea => ContentIdea.create({
          title: idea.title,
          description: idea.description,
          content_type: idea.content_type,
          keywords: idea.keywords,
          source: idea.source_type === 'reddit' ? 'trending' : 'ai_suggested',
          status: 'pending',
          priority: idea.engagement === 'high' ? 5 : idea.engagement === 'low' ? 2 : 3,
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }) => ContentIdea.update(id, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['content-ideas'] });
    },
  });
  ...
  const handleGenerateArticle = (idea) => {
    // Navigate to ArticleWizard with pre-selected data
    const keywords = Array.isArray(idea.keywords) ? idea.keywords.join(', ') : '';
    navigate(`/article-wizard?ideaId=${idea.id}&title=${encodeURIComponent(idea.title)}&type=${idea.content_type}&keywords=${encodeURIComponent(keywords)}&auto=true`);
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="queue" className="mt-6 space-y-4">
            {isLoading ? (
              <Card className="p-8 text-center border-none shadow-lg bg-white">
                      ...
                      <div className="flex items-start justify-between gap-6">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 flex-wrap mb-3">
                            ...
                            <Badge variant="outline" className={`${priorityColors[idea.priority || 3]} border`}>
