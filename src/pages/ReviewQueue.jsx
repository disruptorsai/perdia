...
import { Article, ArticleRevision } from "@/lib/perdia-sdk";
import {
  Clock,
  CheckCircle2,
  AlertCircle,
...
export default function ReviewQueue() {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ['articles', 'review', selectedStatus],
    queryFn: async () => {
  ...
  const { data: allRevisions = [] } = useQuery({
    ...
    queryFn: async () => {
      const result = await ArticleRevision.find({}, {
        orderBy: { column: 'created_date', ascending: false },
        limit: 500
      });
      return result;
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }) => Article.update(id, { status }),
    onSuccess: () => {
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between gap-6">
                        ...
                        <div className="flex flex-col gap-2 flex-shrink-0">
                          <Link to={`/article-editor?id=${article.id}`}>
